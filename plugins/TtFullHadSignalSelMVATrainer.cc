...
TtFullHadSignalSelMVATrainer::analyze(const edm::Event& evt, const edm::EventSetup& setup)
{
  ...
  if(whatData_==-1) {
    ...
    evaluateTtFullHadSignalSel(mvaComputer, selection, weight, true, isSignal);
  }
  else {
    ...
    if(whatData_==1){
      ...
      evaluateTtFullHadSignalSel(mvaComputer, selection, weight, true, true);
    }
    else if(whatData_==0){
      //your tree contains only background events

      evaluateTtFullHadSignalSel(mvaComputer, selection, weight, true, false);
    }
