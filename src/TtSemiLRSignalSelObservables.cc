...
void TtSemiLRSignalSelObservables::operator() (TtSemiEvtSolution &TS,const std::vector<TopJet>& SelectedTopJets)
{
...
// Observables related to the b-disc (jets ordered in Bdisc)
	...
	double BGap = TopJets[1].getBDiscriminator(BtagAlgo) - TopJets[2].getBDiscriminator(BtagAlgo);

	// Sum of the bdisc of the two highest/lowest jets
	double BjetsBdiscSum = TopJets[0].getBDiscriminator(BtagAlgo) + TopJets[1].getBDiscriminator(BtagAlgo);
	double LjetsBdiscSum = TopJets[2].getBDiscriminator(BtagAlgo) + TopJets[3].getBDiscriminator(BtagAlgo);

	Obs8  = (TopJets[2].getBDiscriminator(BtagAlgo) > -10 ? log(BGap) : -5);
